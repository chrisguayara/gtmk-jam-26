...
@export var spin_speed_min: float = 1.5
@export var spin_speed_max: float = 3.5
...
func throw(direction: Vector2, throw_power: float) -> void:
	...
	# Positive rotation means clockwise in Godot's 2D coordinate system.
	_spin_speed = lerpf(
		spin_speed_min,
		spin_speed_max,
		throw_power
...
func _physics_process(delta: float) -> void:
	...
	# Spin only the artwork. The circular collision shape stays unchanged.
